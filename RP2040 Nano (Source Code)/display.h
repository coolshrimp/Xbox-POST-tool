...
#define OLED_RESET    -1
#define PIN_SDA_DISP 10       // Replace with correct GPIO for display SDA
#define PIN_SCL_DISP 11       // Replace with correct GPIO for display SCL
...
class Display {
public:
    Display(uint8_t displayWidth, uint8_t displayHeight, uint8_t i2cAddress) {        
        width = displayWidth;
        height = displayHeight;
        currentRotation = DISPLAY_LANDSCAPE;
        display = Adafruit_SSD1306(width, height, &Wire1, OLED_RESET);
